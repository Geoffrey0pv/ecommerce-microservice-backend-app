...
###############################################################################
# Generador de Pipelines con Kubernetes Plugin
# 
# Uso: ./generate-k8s-plugin-pipeline.sh <service-name> <service-port>
# 
# Ejemplos:
#   ./generate-k8s-plugin-pipeline.sh payment-service 8400
#   ./generate-k8s-plugin-pipeline.sh product-service 8500
#   ./generate-k8s-plugin-pipeline.sh order-service 8300
###############################################################################
...
if [ "$#" -ne 2 ]; then
    ...
    echo ""
    echo "Ejemplos:"
    echo "  $0 payment-service 8400"
    echo "  $0 product-service 8500"
    echo "  $0 order-service 8300"
    echo "  $0 favourite-service 8800"
    exit 1
fi
...
echo -e "${YELLOW}🚀 Generando pipeline con Kubernetes Plugin${NC}"
echo "  Service: $SERVICE_NAME"
echo "  Port: $SERVICE_PORT"
echo ""

# Archivo de salida
OUTPUT_FILE="jenkins-pipelines/${SERVICE_NAME}-stage-pipeline-k8s-plugin.groovy"
...
sed -i "s/SERVICE_NAME_PLACEHOLDER/${SERVICE_NAME}/g" "$OUTPUT_FILE"
sed -i "s/SERVICE_PORT_PLACEHOLDER/${SERVICE_PORT}/g" "$OUTPUT_FILE"

echo -e "${GREEN}✅ Pipeline generado: $OUTPUT_FILE${NC}"
echo ""
echo -e "${YELLOW}📋 Próximos pasos:${NC}"
echo "1. Revisa el archivo generado"
echo "2. Commit y push: git add $OUTPUT_FILE && git commit -m 'feat: add K8s plugin pipeline for ${SERVICE_NAME}'"
echo "3. Configura el job en Jenkins apuntando a este archivo"
echo ""
