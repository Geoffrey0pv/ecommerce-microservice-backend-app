...
// OBJETIVO: Desplegar y Probar un artefacto (imagen) YA EXISTENTE en Staging.
pipeline {
    agent any
    ...
    stages {
        stage('Checkout & Identify Image') {
            steps {
                checkout scm
                script {
                    // 1. Identifica la imagen que se va a desplegar basándose en el commit
            ...
            steps {
                script {
                    ...
                    sh '''
                        ...
                        # Esto crea los recursos si no existen o los actualiza (excepto la imagen)
