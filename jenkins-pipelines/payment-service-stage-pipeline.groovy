pipeline {
    ...
    environment {
        ...
        K8S_DEPLOYMENT_NAME = "payment-service"
        K8S_CONTAINER_NAME = "payment-service"
        K8S_SERVICE_NAME = "payment-service"
        SERVICE_PORT = "8400" 
                ...
                script {
                    sh """
                        ...
                        gcloud artifacts docker images describe \${FULL_IMAGE_NAME}:\${IMAGE_TAG} || {
                            ...
                            echo "Asegúrate de que el pipeline de DEV ('payment-service-pipeline.groovy') haya corrido exitosamente."
                            exit 1
                        }
            ...
            steps {
                script {
                    sh """
                        ...
                        helm upgrade --install \${K8S_DEPLOYMENT_NAME} manifests-gcp/payment-service/ \
                ...
                script {
                    sh """
                        ...
                        kubectl exec \$POD_NAME -n \${K8S_NAMESPACE} -- \
                            curl -f http://localhost:\${SERVICE_PORT}/payment-service/actuator/health || {
